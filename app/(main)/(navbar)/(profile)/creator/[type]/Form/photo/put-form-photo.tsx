...
const FormPutPhoto = ({
  ...
  const { UpdatePhotoData, putPhoto, publicId, isLoadingUpdatePhoto } =
  ...
  useEffect(() => {
    ...
    if (UpdatePhotoData) {
      reset({
        imageName: UpdatePhotoData[0]?.imageName || "",
        imagePath: UpdatePhotoData[0]?.url || "",
        // folderName: UpdatePhotoData[0]?.folderName || "",
        hashtag: UpdatePhotoData[0]?.hashtag || [],
        category: UpdatePhotoData[0]?.category || [],
        description: UpdatePhotoData[0]?.description || "",
      });
    }
  }, [UpdatePhotoData, reset]);

  const submit = handleSubmit(async (values) => {
    try {
      const payload = {
        iuProduct: UpdatePhotoData[0].iuProduct,
        publicId: publicId,
        description: values.description,
        imageName: values.imageName,
        imagePath: values.imagePath,
        prevImage: UpdatePhotoData[0]?.url,
  ...
  return (
    <>
      {Array.isArray(UpdatePhotoData) && (
        <div className="overlay">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-3xl p-6 relative">
            {/* Close button */}
            <button
              type="button"
              onClick={() => setIsRender({ iuProduct: null, value: "" })}
