...
export const ItemFolderDescription = async (id: number) => {
  const query = await prisma.$queryRaw<
    ItemFolderDescriptionType[]
  >`SELECT upi.ref_id_product, upi.description, upi.url, upi.hashtag, upi.category, COALESCE(SUM(upiv.like), 0)::int AS total_like, COALESCE(SUM(upiv.dislike), 0)::int AS total_dislike, up.created_at
    FROM users_product_image upi
